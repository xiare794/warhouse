<?php
	...
	if(isset($_POST['edit'])){
		...
		$updateOuput = update('wApplications',$attrs, $vals);
		...
		$actContent = $_SESSION['user']."对".$InStockID."的.".$_POST['appType']."库单进行修改操作;".$updateOuput;
	...
	if(isset($_POST['delete'])){
		$attrs="appID";
		$vals =$_POST['appID'];
		delete('wapplications',$attrs, $vals);
		//考虑删除货物包
		//deleteWarePackages
	...
	if(isset($_POST['new'])){
		...
		if ($result = mysqli_query($connection, $query)) {
			if($result){
				$rcappid = mysqli_insert_id($connection);
				echo "编号".$rcappid. "的库单(" .$_POST['appName']. ")已添加";
